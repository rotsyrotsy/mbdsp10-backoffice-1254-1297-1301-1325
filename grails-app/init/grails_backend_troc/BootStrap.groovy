    ...
    void initData(){
        Role.withTransaction {
            ["ROLE_SUPER_ADMIN", "ROLE_ADMIN", "ROLE_USER"].each {
                String name ->
                    new Role(role_name: name).save()
            }
        }
        assert Role.count() == 3
        User.withTransaction {
            // Create super admin user and assign the role
            def sadminUser = new User(username: 'super admin', password: 'superadmin', email: 'superadmin@mail.com', role_id: Role.findByRole_name('ROLE_SUPER_ADMIN'))
            if (!sadminUser.save(failOnError: true)) {
                sadminUser.errors.allErrors.each { println it }
            }

            // Create admin user and assign the role
            def adminUser = new User(username: 'admin', password: 'admin', email: 'admin@mail.com', role_id: Role.findByRole_name('ROLE_ADMIN'))
            if (!adminUser.save(failOnError: true)) {
                adminUser.errors.allErrors.each { println it }
            }

            // Create regular user and assign the role
            def userUser = new User(username: 'user', password: 'user', email: 'user@mail.com', role_id: Role.findByRole_name('ROLE_USER'))
